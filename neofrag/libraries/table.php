...
class Table extends Library
{
	...
	public function display()
	{
		...
		else
		{
			...
			if ($this->_pagination && !empty($this->output->module()->pagination) && $this->output->module()->pagination->count() > 10)
			{
				$output .= '<div class="form-group float-left">
								<select class="form-control" style="width: auto;" onchange="window.location=\''.url($this->output->module()->pagination->get_url()).'/\'+$(this).find(\'option:selected\').data(\'url\')" autocomplete="off">
									<option value="10"'. ($this->output->module()->pagination->get_items_per_page() == 10  ? ' selected="selected"' : '').' data-url="page/1/10">'.HB()->lang('%d r̩sultat|%d r̩sultats', 10, 10).'</option>
									<option value="25"'. ($this->output->module()->pagination->get_items_per_page() == 25  ? ' selected="selected"' : '').' data-url="page/1/25">'.HB()->lang('%d r̩sultat|%d r̩sultats', 25, 25).'</option>
									<option value="50"'. ($this->output->module()->pagination->get_items_per_page() == 50  ? ' selected="selected"' : '').' data-url="page/1/50">'.HB()->lang('%d r̩sultat|%d r̩sultats', 50, 50).'</option>
									<option value="100"'.($this->output->module()->pagination->get_items_per_page() == 100 ? ' selected="selected"' : '').' data-url="page/1/100">'.HB()->lang('%d r̩sultat|%d r̩sultats', 100, 100).'</option>
			...
			$output .= '<i>'.HB()->lang('%d r̩sultat|%d r̩sultats', $count, $count).($count < $count_results ? HB()->lang(' sur %d au total', $count_results) : '').'</i>';
