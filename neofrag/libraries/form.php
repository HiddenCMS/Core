...
class Form extends Library
{
	...
	private function _check_text($post, $var, $options)
	{
		...
		{
			return HB()->lang('La valeur s̩lectionn̩e n\'est pas valide|Les valeurs s̩lectionn̩es ne sont pas valides', count($post[$var]));
		}
	...
	public function display()
	{
		...
		{
			$output .= '<div class="form-group row"><div class="offset-3 col-9"><em class="text-muted">'.HB()->lang('* Toutes les informations marqu̩es d\'une ̩toile sont requises').'</em></div></div>';
		}
	...
	private function _display_text($var, $options, $post, $type = 'text')
	{
		...
		if ($type == 'file')
		{
			...
			$input = '<div style="margin: 7px 0;"><p>'.icon('fas fa-download').' '.HB()->lang('T̩l̩charger un fichier').(!empty($options['info']) ? $options['info'] : '').'</p>'.$input.'</div>';
	...
	private function _display_iconpicker($var, $options, $post)
	{
		HB()	->css('bootstrap-iconpicker.min')
					...
					->js_load('	$(".btn.iconpicker").iconpicker({
									...
									labelFooter: "<div class=\"float-right\">'.HB()->lang('{2} ic̫nes').'</div>",
