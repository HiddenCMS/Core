...
function strtolink($string, $is_html = FALSE)
{
	...
	return preg_replace_callback('_@((?:&quot;(.+?)&quot;)|([^@\s]+))_', function($match){
		...
		if ($users === NULL)
		{
			foreach (Hidden()->db->select('id', 'username')->from('user')->where('deleted', FALSE)->get() as $user)
			{
		...
		return ($user_id = array_search($username, $users)) !== FALSE ? Hidden()->user->link($user_id, $username, '@') : $match[0];
	}, $string);
}
...
{
	return nl2br(strtolink(Hidden()->bbcode->bbcode2html($string), TRUE));
}
