...
class Debug extends Core
{
	...
	public function __construct($config = [])
	{
		...
		set_error_handler(function($errno, $errstr, $errfile, $errline){
			...
			{
				if (HIDDEN_DEBUG_BAR || HIDDEN_LOGS)
				{
		...
		if (HIDDEN_LOGS)
		{
			$this->on('output_rendered', function(){
				...
				{
					array_unshift($args, $date->format('Y-m-d H:i:s.u'), sprintf('%.3f', ($memory - HIDDEN_MEMORY) / 1024 / 1024).'Mb', strtoupper($type));
