...
class Hidden
{
	...
	public function __path($type, $file = '', &$dir = [], $callback = 'check_file')
	{
		...
		foreach ($paths[get_called_class()]($this, $type, $file) as $path)
		{
			if ($callback($path))
			{
				if ((HIDDEN_DEBUG_BAR || HIDDEN_LOGS) && isset($this->debug) && !is_a($path, 'HD\Hidden\Libraries\Date'))
				{
		...
		{
			return $Hidden->__path($type, $file, $dir, $callback);
		}
	}
	...
	public function ___load($type, $name, $args = [])
	{
		...
		$callback = function(&$path) use ($args){
			if (class_exists($path = class_name('HD\\'.str_replace('/', '\\', $path))))
			{
				$path = Hidden($path, $args);
				return TRUE;
			}
		};
	...
	public function __get($name)
	{
		...
		{
			if (!defined('HIDDEN_CORE') && substr($name, 0, 5) == 'core_')
			{
				$type = 'core';
				$name = substr($name, 5);
				$args = [];
			...
			{
				$type = 'libraries';
				$args = [property_exists($this, '__caller') ? $this->__caller : (is_a($this, 'HD\Hidden\Loadables\Addon') ? $this : Hidden())];
			}
			...
			if (!is_null($name) && !($object = @Hidden()->___load($type, $name, $args)))
			{
				...
				$object = @Hidden()->___load($type, implode('\\', $class), $args);
			}
	...
	public function __call($name, $args)
	{
		...
		else if (is_a($class = 'HD\Hidden\Displayables\\'.$name, 'HD\Hidden\Displayable', TRUE))
		{
			$callback = Hidden($class);
		}
		else if (is_a($class = 'HD\Hidden\Addons\\'.$name, 'HD\Hidden\Loadable', TRUE))
		{
			return forward_static_call_array($class.'::__load', [Hidden(), $args]);
		}
		else if (is_a($class = 'HD\Hidden\Loadables\\'.$name, 'HD\Hidden\Loadable', TRUE))
		{
			return forward_static_call_array($class.'::__load', [property_exists($this, '__caller') ? $this->__caller : (is_a($this, 'HD\Hidden\Loadables\Addon') ? $this : Hidden()), $args]);
		}
