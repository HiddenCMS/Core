...
abstract class Model2 extends Hidden implements \HD\Hidden\Loadable
{
	...
	public function __toArray($objects = [])
	{
		...
		foreach ($values as &$value)
		{
			...
			else if (is_a($value, 'HD\Hidden\Loadables\Model2') && !$value())
			{
				$value = NULL;
			}
	...
	{
		$this->_id      = 0;
		$this->_updates = $this->_updates + $this->_data;
	...
	public function action($action)
	{
		...
		if ($caller == Hidden() && $this->__name == 'user')
		{
			$caller = Hidden()->module($this->__name);
		}
	...
	public function tracking(&$unread = FALSE)
	{
		...
		{
			$this->__tracking = Hidden()	->db()
											->select('COALESCE(model_id, 0)', 'date')
											->from('tracking')
											->where('user_id',  Hidden()->user->id)
											->where('model',    $this->__table)
											->where('model_id', $this->id, 'OR', 'model_id', NULL)
											->index();
		}

		if (!array_key_exists($this->id, $this->__tracking) && $this->date->diff(Hidden()->user->registration_date) > 0 && (!array_key_exists(0, $this->__tracking) || $this->date->diff($this->__tracking[0]) > 0))
		{
			$unread = TRUE;
		...
		if (!$this->_data)
		{
			...
			if (($auto_id = $this->_db()->insert($this->_table(), $values)) !== NULL)
			{
				foreach ($this->_schema() as $name => $field)
				{
					...
					{
						$this->_data[$field->i] = Hidden()	->model2('i18n')
	...
	{
		return Hidden()->db(defined('static::DB') ? static::DB : 'default');
	}
