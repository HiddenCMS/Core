...
abstract class Labelable extends Library
{
	...
	public function __toString()
	{
		...
		return parent	::html()
						...
						->content($this	->array
										->append_if(($label = (string)$this->_label()) && !($display & Form2::FORM_COMPACT), function() use ($label){
											return parent	::html(($multiple = is_a($this, 'HD\Hidden\Libraries\Forms\Multiple')) ? 'legend' : 'label')
															->attr('class', 'col-form-label')
															->attr_if(!$multiple, 'for', $this->_form->token().'_'.$this->_name)
															->content($label);
										})
										->append($input)
										->append_if($this->_errors && ($display & Form2::FORM_COMPACT), function(){
