...
class Array_ extends Library implements Iterator, ArrayAccess
{
	...
	public function __invoke($array = [])
	{
		...
		else if (is_a($array, 'HD\Hidden\Libraries\Array_') && !is_a($this, 'HD\Hidden\Displayable'))
		{
			return $array;
		}
	...
	public function index($key = NULL)
	{
		...
		{
			if (is_a($row, 'HD\Hidden\Loadables\Model2'))
			{
	...
	protected function _browse()
	{
		...
		while (($name = current($index)) !== FALSE && ($n === NULL || $i++ < $n))
		{
			...
			if (is_a($node[$name], 'HD\Hidden\Libraries\Array_'))
			{
