...
class Modal extends Library
{
	...
	public function __invoke($title, $icon = '')
	{
		...
		$this->_header = is_a($title, 'HD\Hidden\Libraries\Label') ? $title : $this->label($title, $icon);
	...
	{
		if (!is_a($button, 'HD\Hidden\Libraries\Button'))
		{
	...
	{
		Hidden()->js_load('$(\'#'.$this->id.'\').modal(\'show\');');
		return $this;
	}
	...
	{
		if (!is_a($callback, 'HD\Hidden\Libraries\Form2'))
		{
			$callback = $this	->form2()
