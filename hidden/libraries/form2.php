...
class Form2 extends Library
{
		...
		if ($this->_check === NULL)
		{
			...
			$filters = function($data = NULL, &$session = []) use ($load_filters){
				...
				if ($load_filters)
				{
					...
					foreach ($this->_rules as $rule)
					{
						if ((is_string($rule) || is_object($rule)) && method_exists($rule, 'name'))
						{
							$name = $rule->name();
							...
							if ((is_string($rule) || is_object($rule)) && method_exists($rule, 'value') && $data !== NULL)
							{
							...
							if ((is_string($rule) || is_object($rule)) && method_exists($rule, 'filter') && ($filter = $rule->filter()))
							{
								$filters = TRUE;
			...
			if ($this->_check)
			{
				...
				foreach ($this->_rules as $rule)
				{
					if ((is_string($rule) || is_object($rule)) && method_exists($rule, 'check'))
					{
				...
				if ($success)
				{
					...
					if ($this->_success)
					{
						if (is_a($this->_values, 'HD\Hidden\Loadables\Model2'))
						{
	...
	public function form(...$args)
	{
		...
		$this->_is_model = is_a($model, 'HD\Hidden\Loadables\Model2');
	...
	{
		$this->_rules[] = is_a($legend, 'HD\Hidden\Libraries\Forms\Legend') ? $legend : $this->form_legend($legend, $icon);
		return $this;
	}

	public function info($info)
	{
		$this->_rules[] = is_a($info, 'HD\Hidden\Libraries\Forms\Content') ? $info : $this->form_info($info);
		return $this;
	}
	...
	protected function _has_submit()
	{
		foreach ($this->_buttons as $button)
		{
			if (is_a($button, 'HD\Hidden\Libraries\Buttons\Submit'))
			{
				return TRUE;
			}
		}
	...
	protected function _exec()
	{
		if ($this->_values)
		{
			foreach ($this->_rules as $rule)
			{
				if ((is_string($rule) || is_object($rule)) && method_exists($rule, 'value'))
				{
					...
					if (is_a($value, 'HD\Hidden\Loadables\Model2') && !is_a($value, 'HD\Hidden\Models\I18n'))
					{
						$value = $value->id;
					}
		...
		{
			if (!$has_upload && is_a($rule, 'HD\Hidden\Libraries\Forms\File'))
			{
				$has_upload = TRUE;
			}
		...
		foreach ($rules as $rule)
		{
			if ($this->_read_only && ((is_string($rule) || is_object($rule)) && method_exists($rule, $method = 'disabled') || method_exists($rule, $method = 'read_only')))
			{
				$rule->$method();
			}
			...
			if ((is_string($rule) || is_object($rule)) && method_exists($rule, 'size') && $rule->size())
			{
				...
				if (!is_a($last, 'HD\Hidden\Libraries\Html'))
				{
