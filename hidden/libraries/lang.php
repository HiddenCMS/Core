...
class Lang extends Library
{
	...
	static protected function _get($caller, $language, $locale, $key)
	{
		...
		if (!isset($locales[$caller_name = get_class($caller)][$key]))
		{
			...
			foreach (Hidden()->config->langs ?: [Hidden()->config->lang] as $lang)
			{
				$result = NULL;
	...
	public function __toString()
	{
		if ($this->_locale === NULL)
		{
			$locale   = $this->_name;
			$args     = $this->_args;
			$language = isset($this->__caller) && is_a($this->__caller, 'HD\Hidden\Loadables\Addon') && !empty($this->__caller->info()->language) ? $this->__caller->info()->language : 'fr';
			...
			{
				if (HIDDEN_LOGS_I18N && $this->db()->from('log_i18n')->where('language', $language)->where('key', $key)->where('file', $class = get_class($this->__caller))->empty())
				{
					Hidden()->model2('log_i18n')->set('language', $language)->set('key', $key)->set('locale', $locale)->set('file', $class)->create();
				}
			...
			if (HIDDEN_LOGS_I18N)
			{
				if (!isset($lang))
				{
					$lang = Hidden()->model2('addon')->get('language', $language);
				}
