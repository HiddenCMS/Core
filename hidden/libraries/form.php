...
class Form extends Library
{
	...
	static private function _token($id)
	{
		...
		if ($tokens === NULL)
		{
			$tokens = Hidden()->session('form') ?: [];
		}

		if (empty($tokens[$id]))
		{
			Hidden()->session->set('form', $id, $tokens[$id] = unique_id(array_merge([$id], $tokens)));
		}
			...
			if ($this->_has_upload())
			{
				...
				foreach ($this->_rules as $var => $options)
				{
					if (isset($options['type']) && $options['type'] == 'file')
					{
						...
						{
							Hidden()->model2('file', $options['value'])->delete();
							$options['value'] = $post[$var] = 0;
						}

						if (!empty($files['tmp_name'][$var]))
						{
							if (!($post[$var] = Hidden()->model2('file')->static_uploaded_file($files, isset($options['upload']) ? $options['upload'] : NULL, isset($options['value']) ? $options['value'] : NULL, $var)->id))
							{
								$this->_errors[$var] = Hidden()->lang('Erreur de transfert');
								return FALSE;
							}
	...
	private function _check_text($post, $var, $options)
	{
		...
		{
			return Hidden()->lang('La valeur sélectionnée n\'est pas valide|Les valeurs sélectionnées ne sont pas valides', count($post[$var]));
		}
		...
		{
			return Hidden()->lang('Veuillez remplir ce champ');
		}
		...
		if ($is_file && !empty($_FILES[$this->token()]['error'][$var]) && $_FILES[$this->token()]['error'][$var] != 4)
		{
			...
			return Hidden()->lang($errors[$_FILES[$this->token()]['error'][$var]]);
		}
	...
	private function _check_email($post, $var, $options)
	{
		...
		{
			return Hidden()->lang('Veuillez entrer une adresse email valide');
		}
	...
	private function _check_url($post, $var, $options)
	{
		...
		{
			return Hidden()->lang('Veuillez entrer une adresse url valide');
		}
	...
	public function display()
	{
		if ($this->_confirm_deletion)
		{
			...
			if ($this->url->ajax())
			{
				return '<div class="modal-header">
							...
							<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">'.Hidden()->lang('Fermer').'</span></button>
						</div>
						<div class="modal-body">
							'.$message.'
						</div>
						<div class="modal-footer">
							<button type="button" class="btn btn-secondary" data-dismiss="modal">'.Hidden()->lang('Annuler').'</button>
							<a class="btn btn-danger delete-confirm" href="'.url($this->url->request).'" data-form-id="'.$this->token().'" onclick="return confirm_deletion(this);">'.Hidden()->lang('Supprimer').'</a>
						</div>';
			}
			else
			{
		...
		{
			Hidden()->js('https://www.google.com/recaptcha/api.js?hl='.$this->config->lang->info()->name.'&_=');
		...
		{
			$output .= '<div class="form-group row"><div class="offset-3 col-9"><em class="text-muted">'.Hidden()->lang('* Toutes les informations marquées d\'une étoile sont requises').'</em></div></div>';
		}
	...
	private function _display_text($var, $options, $post, $type = 'text')
	{
		...
		if (in_array($type, ['date', 'datetime', 'time']))
		{
			...
			Hidden()	->css('bootstrap-datetimepicker.min')
		...
		else if ($type == 'colorpicker')
		{
			...
			Hidden()	->css('bootstrap-colorpicker.min')
		...
		{
			$output .= '<div class="input-group mb-3'.(!empty($classes) ? ' '.implode(' ', $classes) : '').'">
		...
		if ($type == 'file')
		{
			...
			$input = '<div style="margin: 7px 0;"><p>'.icon('fas fa-download').' '.Hidden()->lang('Télécharger un fichier').(!empty($options['info']) ? $options['info'] : '').'</p>'.$input.'</div>';
									...
									<div class="col-3">
										<div class="thumbnail">
											...
											<div class="caption text-center">
												<a class="btn btn-outline-danger btn-block btn-sm form-file-delete" href="#" data-input="'.$this->token().'['.$var.']">'.icon('far fa-trash-alt').' '.Hidden()->lang('Supprimer').'</a>
											</div>
										</div>
									</div>
									<div class="col-9">
	...
	{
		Hidden()	->css('bootstrap-iconpicker.min')
					...
					->js_load('	$(".btn.iconpicker").iconpicker({
									...
									rows: 5,
									iconset: "fontawesome",
									labelHeader: "'.Hidden()->lang('{0} sur {1} pages').'",
									labelFooter: "<div class=\"float-right\">'.Hidden()->lang('{2} icônes').'</div>",
									searchText: "'.Hidden()->lang('Rechercher...').'",
	...
	private function _display_date($var, $options, $post)
	{
		...
		{
			$options['value'] = timetostr(Hidden()->lang('d/m/Y'), $options['value']);
		}
		else
		{
			$options['value'] = '';
	...
	private function _display_datetime($var, $options, $post)
	{
		if (isset($options['value']) && $options['value'] !== '')
		{
			$options['value'] = timetostr(Hidden()->lang('d/m/Y H:i'), $options['value']);
		}
		else
		{
			$options['value'] = '';
	...
	private function _display_time($var, $options, $post)
	{
		...
		{
			$options['value'] = timetostr(Hidden()->lang('H:i'), $options['value']);
		}
		else
		{
			$options['value'] = '';
	...
	private function _display_select($var, $options, $post)
	{
		...
		$output = '<select class="input-group mb-3" id="form_'.$this->token().'_'.$var.'" name="'.$this->token().'['.$var.']">
						<option></option>';
	...
	{
		return '<textarea id="form_'.$this->token().'_'.$var.'" class="input-group'.($editor ? ' editor' : '').'" rows="10" name="'.$this->token().'['.$var.']">'.$this->_display_value($var, $options).'</textarea>';
	}
