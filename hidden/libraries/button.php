...
class Button extends Label
{
		...
		if ($buttons)
		{
			...
			else
			{
				$footers->each(function($buttons, $align){
					return Hidden()->html()->attr('class', 'float-'.$align)->content($buttons);
				});
			}
	...
	{
		call_user_func_array('HD\\Hidden\\Libraries\\Label::__invoke', func_get_args());
	...
	{
		$modal = is_a($title, 'HD\Hidden\Libraries\Modal') ? $title : parent::modal($title, $icon);
