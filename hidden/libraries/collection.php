...
class Collection extends Library
{
	...
	public function __invoke($model = NULL)
	{
		if ($model)
		{
			if (!is_a($model, 'HD\Hidden\Loadables\Model2'))
			{
	...
	public function __call($name, $args)
	{
		if ($name != 'clone' && $this->_db)
		{
			...
			else
			{
				...
				return is_a($result, 'HD\Hidden\Hidden') ? $this : $result;
			}
		}
	...
	public function paginate($page, $limit = 20)
	{
		...
		$this->pagination = Hidden()->pagination($this->_db, $page, $limit);
	...
	public function tracking()
	{
		...
		if ($count && !$total_unread && Hidden()->db()->from('tracking')->where('user_id', Hidden()->user->id)->where('model', $this->_model->__table)->count() > 1)
		{
			Hidden()	->collection('tracking')
