...
class File extends Model2
{
	static public function __schema()
	{
		return [
			...
			'user' => self::field()->depends('user/user')->default(Hidden()->user)->null(),
	...
	static public function uploaded_file($files, $dir = NULL, $file_id = NULL, $var = NULL)
	{
		...
		{
			if (($file = Hidden()->model2('file', $file_id)) && $file->id)
			{
				@unlink($file->path);

				return $file->set('user', Hidden()->user)
