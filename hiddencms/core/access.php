...
class Access extends Core
{
	...
	public function count($module, $action, $id = 0)
	{
		...
		if (!empty($count[1]))
		{
			$output[] = '<span class="text-success" data-toggle="tooltip" title="'.$this->lang('Authorized members').'" data-original-title="">'.icon('fas fa-check').' '.$count[1].'</span>';
		}

		if (!empty($count[0]))
		{
			$output[] = '<span class="text-danger" data-toggle="tooltip" title="'.$this->lang('Excluded members').'">'.icon('fas fa-ban').' '.$count[0].'</span>';
		}

		if (!$this($module, $action, $id, 'visitors'))
		{
			$output[] = '<span class="text-info" data-toggle="tooltip" title="'.$this->lang('Excluded visitors').'">'.icon('far fa-eye-slash').'</span>';
		}
