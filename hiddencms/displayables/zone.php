		...
		if ($live_editor = HB()->output->live_editor())
		{
			...
			if ($live_editor & \HB\HiddenCMS\Core\Output::ZONES)
			{
				...
				$output = '	<div class="float-right">
								'.($disposition['page'] == '*' ? '<button type="button" class="btn btn-link live-editor-fork" data-enabled="0">'.icon('fas fa-toggle-off').' '.HB()->lang('Disposition commune').'</button>' : '<button type="button" class="btn btn-link live-editor-fork" data-enabled="1">'.icon('fas fa-toggle-on').' '.HB()->lang('Disposition spÃƒÆ’Ã‚Â©cifique ÃƒÆ’Ã‚Â  la page').'</button>').'
							</div>
