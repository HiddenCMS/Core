...
class HiddenCMS
{
	...
	public function __path($type, $file = '', &$dir = [], $callback = 'check_file')
	{
		...
		if ($this != ($HiddenCMS = HB()))
		{
			return $HiddenCMS->__path($type, $file, $dir, $callback);
		}
	}
	...
	public function ___load($type, $name, $args = [])
	{
		...
		$callback = function(&$path) use ($args){
			if (class_exists($path = class_name('HB\\'.str_replace('/', '\\', $path))))
			{
				$path = HB($path, $args);
				return TRUE;
			}
	...
	public function __get($name)
	{
		...
		{
			if (!defined('HIDDENCMS_CORE') && substr($name, 0, 5) == 'core_')
			{
				$type = 'core';
				$name = substr($name, 5);
				$args = [];
