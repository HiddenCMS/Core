...
<div class="about-content">
	<?php if ($settings['display_teamname'] == 'oui'): ?>
		<?php if ($this->config->team_name): ?><h3 class="<?php echo $settings['teamname_align'] ?>"<?php echo $settings['style_title'] ? ' style="color: '.$settings['style_title'].'"' : '' ?>><?php echo $this->config->team_name ?></h3><?php endif ?>
		...
		<ul class="list-inline <?php echo $settings['teamname_align'] ? $settings['teamname_align'] : 'text-left' ?>"<?php echo $settings['style_title'] ? ' style="color: '.$settings['style_title'].'"' : '' ?>>
			<?php if ($settings['display_type'] == 'oui' && $this->config->team_type): ?><li class="list-inline-item"><?php echo icon('fas fa-university').' '.$this->config->team_type ?></li><?php endif ?>
			<?php if ($settings['display_date'] == 'oui' && $this->config->team_creation): ?><li class="list-inline-item"><?php echo icon('fas fa-calendar-alt').' '.timetostr('j M Y', $this->config->team_creation) ?></li><?php endif ?>
		</ul>
		<?php endif ?>
	<?php endif ?>
	<?php if ($settings['display_logo'] == 'oui' && $this->config->team_logo): ?>
		<div class="<?php echo $settings['logo_align'] ?> mb-3">
			<img src="<?php echo NeoFrag()->model2('file', $this->config->team_logo)->path() ?>" style="max-width: <?php echo $settings['logo_width'] ?>px;" alt="" />
		</div>
	<?php endif ?>
	<?php if ($settings['display_biographie'] == 'oui' && $this->config->team_biographie): ?>
		<div class="<?php echo $settings['biographie_align'] ?>"<?php echo $settings['style_text'] ? ' style="color: '.$settings['style_text'].'"' : '' ?>>
			<?php echo bbcode($this->config->team_biographie) ?>
		</div>
	<?php endif ?>
</div>
