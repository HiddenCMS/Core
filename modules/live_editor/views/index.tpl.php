	...
	<nav class="live-editor-navbar navbar navbar-expand-lg navbar-light bg-light py-2">
		...
		<div class="collapse navbar-collapse" id="modules-links-collapse">
			...
			<ul class="navbar-nav ml-auto">
				<li class="nav-item">
					<div class="btn-group">
						<button type="button" class="btn btn-light live-editor-mode<?php echo $live_editor & Output::ZONES ? ' active' : '' ?>" data-mode="<?php echo Output::ZONES ?>"><?php echo icon('far fa-square').' '.$this->lang('Zones') ?></button>
						<button type="button" class="btn btn-light live-editor-mode<?php echo $live_editor & Output::ROWS ? ' active' : '' ?>" data-mode="<?php echo Output::ROWS ?>"><?php echo icon('fas fa-columns fa-rotate-270').' '.$this->lang('Lignes') ?></button>
						<button type="button" class="btn btn-light live-editor-mode<?php echo $live_editor & Output::COLS ? ' active' : '' ?>" data-mode="<?php echo Output::COLS ?>"><?php echo icon('fas fa-columns').' '.$this->lang('Colonnes') ?></button>
						<button type="button" class="btn btn-light live-editor-mode active" data-mode="<?php echo Output::WIDGETS ?>"><?php echo icon('fas fa-th-large').' '.$this->lang('Widgets') ?></button>
					</div>
				</li>
