...
<form target="live-editor-iframe" action="<?php echo $iframe_url ?>" method="post">
	...
	<nav class="ui borderless menu live-editor-navbar">
		...
		<div class="item live-editor-map-item">
			<span id="live-editor-map" data-outline-mode="1"<?php echo $outline_title !== '' ? ' data-outline-title="'.utf8_htmlentities(icon('fas fa-layer-group').' '.$outline_title).'"' : '' ?>><?php echo $outline_title !== '' ? icon('fas fa-layer-group').' '.$outline_title : icon('fas fa-spinner fa-spin').' '.$this->lang('Loading...') ?></span>
		</div>
		<div class="right menu">
			...
			<button type="button" class="item live-editor-mode<?php echo $live_editor & \HB\HiddenCMS\Core\Output::ROWS ? ' active' : '' ?>" data-mode="<?php echo \HB\HiddenCMS\Core\Output::ROWS ?>"><?php echo icon('fas fa-columns fa-rotate-270').' '.$this->lang('Rows') ?></button>
			<button type="button" class="item live-editor-mode<?php echo $live_editor & \HB\HiddenCMS\Core\Output::COLS ? ' active' : '' ?>" data-mode="<?php echo \HB\HiddenCMS\Core\Output::COLS ?>"><?php echo icon('fas fa-columns').' '.$this->lang('Columns') ?></button>
			...
			<div class="ui dropdown icon item live-editor-screen-picker" role="button" tabindex="0" aria-label="<?php echo $this->lang('Screen size') ?>">
				<span class="live-editor-screen-current"><?php echo icon('fas fa-desktop') ?></span>
				<i class="dropdown icon"></i>
				<div class="menu screen">
					<button type="button" class="item live-editor-screen active" data-width="100%"><?php echo icon('fas fa-desktop') ?><span><?php echo $this->lang('Computer') ?></span></button>
					<button type="button" class="item live-editor-screen" data-width="992px"><?php echo icon('fas fa-tablet-alt fa-rotate-270') ?><span><?php echo $this->lang('Tablet landscape') ?></span></button>
					<button type="button" class="item live-editor-screen" data-width="768px"><?php echo icon('fas fa-tablet-alt') ?><span><?php echo $this->lang('Tablet portrait') ?></span></button>
