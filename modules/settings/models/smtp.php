...
class Smtp extends Model
{
    ...
    public function save($data)
    {
        if (!in_array($data['smtp_enabled'] ?? NULL, ['0', '1'], TRUE) ||
            ...
            filter_var($data['smtp_port'] ?? '', FILTER_VALIDATE_INT, ['options' => ['min_range' => 1, 'max_range' => 65535]]) === FALSE) throw new \InvalidArgumentException('Configuration SMTP invalide.');
        $host = $data['smtp_host'] ?? '';
        if (($data['smtp_enabled'] === '1' && $host === '') || ($host !== '' && !filter_var($host, FILTER_VALIDATE_IP) && !filter_var($host, FILTER_VALIDATE_DOMAIN, FILTER_FLAG_HOSTNAME))) throw new \InvalidArgumentException('Serveur SMTP invalide.');
        if (!empty($data['smtp_from']) && !filter_var($data['smtp_from'], FILTER_VALIDATE_EMAIL)) throw new \InvalidArgumentException('Expéditeur invalide.');
        $secret = NULL;
        ...
        elseif ($data['smtp_password'] !== '') {
            ...
            if ($encrypted === FALSE) throw new \RuntimeException('Impossible de protéger le mot de passe SMTP.');
    ...
    private function key()
    {
        ...
        if (empty($crypt['key'])) throw new \RuntimeException('Clé de chiffrement SMTP indisponible.');
    ...
    private function decrypt($value)
    {
        ...
        if ($raw === FALSE || strlen($raw) < 28) throw new \RuntimeException('Mot de passe SMTP illisible.');
        $password = openssl_decrypt(substr($raw, 28), 'aes-256-gcm', $this->key(), OPENSSL_RAW_DATA, substr($raw, 0, 12), substr($raw, 12, 16));
        if ($password === FALSE) throw new \RuntimeException('Mot de passe SMTP illisible.');
        return $password;
    }
}
